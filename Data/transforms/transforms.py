...
__all__ = ["Compose", "ToTensor", "PILToTensor", "ConvertImageDtype", "ToPILImage", "Normalize", "Resize", "Scale",
           ...
           "RandomSolarize", "RandomAdjustSharpness", "RandomAutocontrast", "RandomEqualize", "AugmentVideo", "ConsistentRandomCrop"]

class ConsistentRandomCrop(torch.nn.Module):
    """

    """
    def __init__(self, center_crop=True, frame_size=112, norm=True, mean=128., std=128., tensor_function=None):
        super(ConsistentRandomCrop, self).__init__()
        self.frame_size = frame_size
        self.center_crop = center_crop
        from torchvision import transforms
        self.norm = transforms.Normalize(mean, std) if norm else None
        self.tensor_function = tensor_function


    def forward(self, clip):
        # randomize params for the clip
        _, height, width = self.tensor_function(clip[0]).shape

        center_crop = True
        crop_pos_x = np.random.randint(0, height - self.frame_size)
        crop_pos_y = np.random.randint(0, width - self.frame_size)
        if center_crop:
            # if we need to crop only from the center of the frame
            crop_pos_y = np.random.randint(50, width - self.frame_size - 50)

        new_clip = []
        for frame in clip:
            frame = self.tensor_function(frame)
            frame = frame[:, crop_pos_x:crop_pos_x+self.frame_size, crop_pos_y:crop_pos_y+self.frame_size]
            if self.norm:
...
class AugmentVideo(torch.nn.Module):
    """
    Applies Temporally Consistent Spatial Transformations based on
    [1]R. Qian et al., “Spatiotemporal Contrastive Video Representation Learning,” arXiv:2008.03800 [cs], Apr. 2021,
    Accessed: May 15, 2021. [Online]. Available: http://arxiv.org/abs/2008.03800
    """
    def __init__(self, mean, std, frame_size=112, tensor_function=None):
        super(AugmentVideo, self).__init__()
        self.frame_size = frame_size
        self.random_resize_crop = RandomResizedCrop(self.frame_size, scale=(0.3,1), ratio=(0.5,2))
        strength = 0
        brightness = 0.8 * strength
        contrast = 0  # 0.8 * strength
        saturation = 0  # 0.8 * strength
        hue = 0.2 * strength
        ...
        self.gaussian_blur = GaussianBlur(self.frame_size//30)
    ...
    def forward(self, clip):
        for i, frame in enumerate(clip):
            clip[i] = self.tensor_function(frame)
        clip=torch.stack(clip)
        # randomize params for the clip
        F_flip = 0 if 0.5 < torch.rand(1) else 1
        F_jitter = 0 if 0.8 < torch.rand(1) else 1
        F_grey = 0 if 0.2 < torch.rand(1) else 1
        i, j, h, w = self.random_resize_crop.get_params(clip, self.random_resize_crop.scale, self.random_resize_crop.ratio)
        sigma = self.gaussian_blur.get_params(self.gaussian_blur.sigma[0], self.gaussian_blur.sigma[1])
        fdx, b, c ,s, hue = self.color_jitter.get_params(self.color_jitter.brightness, self.color_jitter.contrast, self.color_jitter.saturation, self.color_jitter.hue)
        new_clip = []
        for frame in clip:
            frame = F.resized_crop(frame, i, j, h, w, self.random_resize_crop.size, self.random_resize_crop.interpolation)
            frame = F.hflip(frame) if F_flip else frame
            frame = self.color_jitter(frame, fdx, b, c ,s, hue) if F_jitter else frame
            frame = self.grayscale(frame) if F_grey else frame
            frame = F.gaussian_blur(frame, self.gaussian_blur.kernel_size, [sigma, sigma])
