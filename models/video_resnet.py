...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, conv_builder, stride=1, downsample=None, dropout=0.3):
        ...
        if self.dropout > 0:
            # self.drop_layer = nn.Dropout3d(self.dropout)
...
class VideoResNet(nn.Module):
    ...
    def _make_layer(self, block, conv_builder, planes, blocks, stride=1, dropout=0.3):
        downsample = None
        ...
        for i in range(1, blocks):
            layers.append(block(self.inplanes, planes, conv_builder, dropout=dropout))
